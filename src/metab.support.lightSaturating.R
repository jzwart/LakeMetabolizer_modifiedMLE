...
my.metab.mle <- function(do.obs, do.sat, k.gas, z.mix, irr, wtr, irr_day, wtr_day, error.type='OE', logged=T,
  ...
  if(error.type=='OE'){
    if(logged==T){
      guesses <- c(log(guesses[1:3]), guesses[4], log(do.obs[1]))  # exponentiating R and GPP coeff to constrain positive / negative 
    }else{
    ...
    pars0 <- fit$par
    if(logged==T){
      pars <- c("gppMaxCoeff"=exp(pars0[1]), "gppCoeff"=exp(pars0[2]), "rCoeff"=-exp(pars0[3]), "Q"=exp(pars0[4]), "nll"=fit$value, "doInit"=exp(pars0[5]),"converge"=fit$convergence)
    }else{
      pars <- c("gppMaxCoeff"=pars0[1], "gppCoeff"=pars0[2], "rCoeff"=pars0[3], "Q"=exp(pars0[4]), "nll"=fit$value, "doInit"=pars0[5],"converge"=fit$convergence)
    }
    ...
    if(bootstrap){
      ...
      gppMaxCoeff_sd<-sd(bootResults$gppMaxCoeff,na.rm=T)
    ...
    if(logged==T){
      guesses <- c(log(guesses[1:3]), guesses[4]) 
    }else{
      guesses <- c(guesses) 
    }
    ...
    pars0 <- fit$par
    if(logged==T){
      pars <- c("gppMaxCoeff"=exp(pars0[1]), "gppCoeff"=exp(pars0[2]), "rCoeff"=-exp(pars0[3]), "Q"=exp(pars0[4]), "nll"=fit$value,'converge'=fit$convergence)
    }else{
      pars <- c("gppMaxCoeff"=pars0[1], "gppCoeff"=pars0[2], "rCoeff"=pars0[3], "Q"=exp(pars0[4]), "nll"=fit$value,'converge'=fit$convergence)
    }
  ...
  # ====================================
  GPP <- mean(pars[1]*(1-exp(-pars[2]*irr_day/pars[1])), na.rm=TRUE) * freq/length(daysSim)  
  R <- mean(pars[3]*log(wtr_day), na.rm=TRUE) * freq/length(daysSim)
  ...
  if(bootstrap){
    return(list("params"=pars, "metab"=c("GPP"=GPP,"R"=R,"NEP"=GPP+R,"GPP_SD"=GPP_sd,"R_SD"=R_sd,
                                         "gppMaxCoeff_SD"=gppCoeff_sd, "gppCoeff_SD"=gppCoeff_sd, "rCoeff_SD"=rCoeff_sd, "doInit_SD"=doInit_sd,"nll"=nll,"converge"=converge)))
  }else{
...
bootstrap.metab <- function(guesses, pars, fn, do.obs, do.sat, k.gas, z.mix, irr, wtr, error.type='OE', logged=T, n.boot=1000, ar1.resids, freq, ...){
  ...
  result <- data.frame(boot.iter = 1:n.boot, gppMaxCoeff = rep(NA,n.boot), gppCoeff = rep(NA,n.boot),rCoeff = rep(NA,n.boot),doInit = rep(NA,n.boot),nll = rep(NA,n.boot),
  ...
  for(i in 1:n.boot){
    ...
    if(error.type=='OE'){
      ...
      if(logged){
        pars <- c("gppMaxCoeff"=exp(pars0[1]), "gppCoeff"=exp(pars0[2]), "rCoeff"=-exp(pars0[3]), "Q"=exp(pars0[4]), "nll"=boot.temp$value, "doInit"=exp(pars0[5]))
      }else{
        pars <- c("gppMaxCoeff"=pars0[1], "gppCoeff"=pars0[2], "rCoeff"=pars0[3], "Q"=exp(pars0[4]), "nll"=boot.temp$value, "doInit"=pars0[5])
      }
      
      GPP <- mean(pars[1]*(1-exp(-pars[2]*irr_day/pars[1])), na.rm=TRUE) * freq
      R <- mean(pars[3]*log(wtr), na.rm=TRUE) * freq
      NEP<-GPP+R
      # store result of current iteration in output data frame 
      result[i,2:4]<-pars[1:3]
      result[i,5]<-pars[6]
      result[i,6]<-pars[5]
      result[i,7]<-GPP
      result[i,8]<-R
      ...
      if(logged){
        pars <- c("gppMaxCoeff"=exp(pars0[1]), "gppCoeff"=exp(pars0[2]), "rCoeff"=-exp(pars0[3]), "Q"=exp(pars0[4]), "nll"=boot.temp$value)
      }else{
        pars <- c("gppMaxCoeff"=pars0[1], "gppCoeff"=pars0[2], "rCoeff"=pars0[3], "Q"=exp(pars0[4]), "nll"=boot.temp$value)
      }
...
# ============================================
mleLoopOE <- function(alpha, doobs, c1, c2, c3, beta, irr, wtr, kz, dosat){
  nobs <- length(doobs)
  a.loop <- .C("mleLoopLightSatCoe", alpha=as.double(alpha), as.double(doobs), as.double(c1), as.double(c2), as.double(c3), as.double(beta), 
               as.double(irr), as.double(wtr), as.double(kz), as.double(dosat), as.integer(nobs))
  return(a.loop[["alpha"]])
}
...
# ============================================
mleLoopPE <- function(alpha, doobs, c1, c2, c3, beta, irr, wtr, kz, dosat){
  nobs <- length(doobs)
  a.loop <- .C("mleLoopLightSatCpe", alpha=as.double(alpha), as.double(doobs), as.double(c1), as.double(c2), as.double(c3), as.double(beta), 
               as.double(irr), as.double(wtr), as.double(kz), as.double(dosat), as.integer(nobs))
  return(a.loop[["alpha"]])
}
...
mleNllPE <- function(Params, do.obs, do.sat, k.gas, z.mix, irr, wtr,logged){
  if(logged==T){ #exponentiating if logged ==T 
    c1 <- exp(Params[1]) # GPP max coeff 
    c2 <- exp(Params[2]) #PAR coeff
    c3 <- -exp(Params[3]) #log(Temp) coeff; constraining negative 
  }else{
    c1 <- Params[1] # GPP max coeff 
    c2 <- Params[2] #PAR coeff
    c3 <- Params[3] #log(Temp) coeff
  }
  Q <- exp(Params[4]) # Variance of the process error
  ...
  #}
  alpha <- mleLoopPE(alpha=alpha, doobs=do.obs, c1=c1, c2=c2, c3=c3, beta=beta, irr=irr, wtr=wtr, kz=kz, dosat=do.sat)
...
mleNllOE <- function(Params, do.obs, do.sat, k.gas, z.mix, irr, wtr, logged){
  if(logged==T){ #exponentiating if logged ==T 
    ...
    # Set first true value equal to first observation
    alpha <- rep(0, length(do.obs))
    alpha[1] <- exp(Params[5]) #Free varying initial DO value
  }else{
    c1 <- Params[1] # GPP max coeff 
    c2 <- Params[2] #PAR coeff
    c3 <- Params[3] #log(Temp) coeff
    # Set first true value equal to first observation
    alpha <- rep(0, length(do.obs))
    alpha[1] <- Params[5] #Free varying initial DO value
  }
  Q <- exp(Params[4]) # Variance of the process error
  ...
  alpha <- mleLoopOE(alpha=alpha, doobs=do.obs, c1=c1, c2=c2, c3=c3, beta=beta, irr=irr, wtr=wtr, kz=kz, dosat=do.sat)
...
predictDO<-function(par, do.obs, do.sat, k.gas, z.mix, irr, wtr, error.type='OE',logged=T ,...){
  ...
  if(error.type=='PE'){
    if(logged==T){
      c1 <- exp(par[1]) # GPP max coeff 
      c2 <- exp(par[2]) #PAR coeff
      c3 <- -exp(par[3]) #log(Temp) coeff
    }else{
      c1 <- par[1] #GPP max coeff 
      c2 <- par[2] #PAR coeff
      c3 <- par[3] #log(Temp) coeff
    }
    # Set first true value equal to first observation
    alpha <- rep(0, length(do.obs))
    alpha[1] <- do.obs[1]#Let's give this model some starting values
  ...
  if(error.type=='OE'){
    if(logged==T){
      ...
      # Set first true value equal to first observation
      alpha <- rep(0, length(do.obs))
      alpha[1] <- exp(par[4])#Let's give this model some starting values
    }else{
      c1 <- par[1] # GPP max coeff 
      c2 <- par[2] #PAR coeff
      c3 <- par[3] #log(Temp) coeff
      # Set first true value equal to first observation
      alpha <- rep(0, length(do.obs))
      alpha[1] <- par[4]#Let's give this model some starting values
    }
  }
  ...
  for(i in 2:length(do.obs)){
   if(kz[i-1]!=0){
     a1 <- c1*(1-exp(-c2*irr[i-1]/c1)) + c3*log(wtr[i-1]) + kz[i-1]*do.sat[i-1]
     alpha[i] <- a1/kz[i-1] + -exp(-kz[i-1])*a1/kz[i-1] + beta[i-1]*alpha[i-1] # NOTE: beta==exp(-kz); kz=K/Zmix
   }else{
     alpha[i]<-alpha[i-1] + c1*(1-exp(-c2*irr[i-1]/c1)) + c3*log(wtr[i-1])
   }
  }
...
my.metab <- function(data, method = NULL, wtr.name="wtr", irr.name="irr", do.obs.name="do.obs", nDaysSim, sunrise, ...){
